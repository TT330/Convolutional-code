function  [Generator_Matrix,Oct_Generator_Matrix,m] = Recongnize_Generator(Check_Sequence,n,k,m,bitstream)
%function  n = Recongnize_Generator(Check_Sequence,n,k,bitstream)

    if n==2 && k==1
        LEN=length(Check_Sequence(:,1));
        Initial_Generator=zeros(LEN/2,2);
        j=1;
        for i=1:2:LEN-1
           if j<=LEN/2
                Initial_Generator(j,1)=Check_Sequence(i,1);
                Initial_Generator(j,2)=Check_Sequence(i+1,1);   
                j=j+1;
           else
               break;
           end
     ...
     y_2=dec2base(y_2,8);
     y_2=str2num(y_2);
     Oct_Generator_Matrix=[y_1,y_2];
    end
    
   %针对n为偶数 
    if k==1 && n>2
        groups=ceil(n/2);%分成n/2组
        bitstream2=reshape(bitstream,n,[]);%将bit流转换成 n行，方便提取
        ...
        if mod(n,2)==0
            for group=1:groups      
              ...
              New_bitstream2=New_bitstream1(1,1:length_catch);
                ...
                for i=1:49
                N_matrix(i,:)=New_bitstream2(1,1+(i-1)*2 : 1+(i-1)*2+47); %按照方程构造系数矩阵
                end
